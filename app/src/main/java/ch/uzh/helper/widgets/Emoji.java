...
import ch.uzh.helper.AndroidUtilities;
import ch.uzh.GlobalState;
import ch.uzh.helper.NotificationCenter;
...
public class Emoji {
    ...
    private static void loadEmoji(final int page) {
        try {
            ...
            File imageFile = GlobalState.getInstance().getFileStreamPath(imageName);
            if (!imageFile.exists()) {
                InputStream is = GlobalState.getInstance().getAssets().open("emoji/" + imageName);
            ...
            imageFile = GlobalState.getInstance().getFileStreamPath(imageName);
            if (!imageFile.exists()) {
                InputStream is = GlobalState.getInstance().getAssets().open("emoji/" + imageName);
                AndroidUtilities.copyFile(is, imageFile);
                is.close();
